<template>
    <div class="space-y-8 container">
        ...
        <UiLoading v-if="fetchingTools" text="Fetching tools"></UiLoading>

        <UiEmpty v-else-if="fetchingToolsError" message="Tools are currently unavailable">
        </UiEmpty>

        <div v-else-if="tools && tools.length > 0"
            class="flex space-x-4 items-center w-full overflow-y-hidden overflow-x-scroll">
            <UiBadge v-for="(tool, index) in sortedTools" :key="index" :text="tool.title" role="button"
                @click="toggleTool(tool.title)" :selected="isSelectedTool(tool.title)" />
        </div>

        <div class="relative overflow-x-auto">
            <transition name="fade" mode="out-in">

                <SkeletonsProjectsArchiveTable v-if="pending" key="skeletons"></SkeletonsProjectsArchiveTable>

                <UiEmpty v-else-if="error" message="Projects are currently unavailable">
                </UiEmpty>
                ...
                <section v-else key="content" class="space-y-4">
                    <table class="w-full text-sm text-left rtl:text-right text-white"
                        ...
                        <tbody>
                            <tr class="border-b border-smoky items-center" v-for="(project, index) in projects"
                                :key="index">
                    ...
                    <UiEmpty v-else :message="selectedTools.length > 0
